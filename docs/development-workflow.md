...
初学者が実装を担当するIssueでは、Specの承認後、実装ファイルを作成または変更する前に、初心者向けの説明フェーズを必ず設ける。

説明では、次の内容を扱う。

- 作業の目的と完了条件。
- 初めて出る用語の意味と、それぞれの役割。
- 作成または変更するファイルと、変更する理由。
- 実装と確認を行う順番。
- 実行するコマンド、成功時の状態、失敗時に確認する場所、後片付けの方法。

説明を受けたユーザーが実装を進める意思を示してから、実装を始める。

この説明フェーズはSpecの承認を置き換えない。

Specの承認は作る内容を決めるために行い、説明フェーズは実装する内容を理解して進めるために行う。
...
- 初学者が実装する場合、Spec承認後に初心者向け説明フェーズを実施している。
- 完了条件がチェックリストで書かれている。
